...
{
#ifdef PROFILE
  FunctionProfiler fp(L"synth_2to1_8bit");
#endif
...
{
#ifdef PROFILE
  FunctionProfiler fp(L"synth_2to1_8bit_mono");
#endif
...
{
#ifdef PROFILE
  FunctionProfiler fp(L"synth_2to1_8bit_mono2stereo");
#endif
...
{
#ifdef PROFILE
  FunctionProfiler fp(L"synth_2to1_mono");
#endif
...
{
#ifdef PROFILE
  FunctionProfiler fp(L"synth_2to1_mono2stereo");
#endif
...
#if defined(AUTO_PC) && !defined(DEBUG)
extern "C" {
...
{
#ifdef PROFILE
  FunctionProfiler fp(L"synth_2to1");
#endif
...
//  static int bo = 1;
  ...
  // Zero out the buffer so that it won't have to be zero'd when downsampling.
  ...
  if(bo & 0x1) {
    b0 = buf[0];
    bo1 = bo;
    dct64_2to1(buf[1]+((bo+1)&0xf),buf[0]+bo,bandPtr);
  }
  else {
    b0 = buf[1];
    bo1 = bo+1;
    dct64_2to1(buf[0]+bo,buf[1]+bo+1,bandPtr);
  }
