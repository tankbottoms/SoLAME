...
{
public:
				ID3_Tag							( IDssInputStream* pIStream = NULL );
				~ID3_Tag						( void );
...
luint			Link							( IDssInputStream* pIStream );
void			Update							( void );
...
void			SetupTag						( IDssInputStream* pIStream );
...
IDssInputStream* m_pIStream;
#ifdef UNDER_CE
//HANDLE			fileHandle;
#else
//FILE			*fileHandle;					// a handle to the file we are linked to
#endif
TCHAR			fileName[ 256 ];				// the name of the file we are linked to
TCHAR			tempName[ 256 ];				// a temp name in case we need a temp file for copying stuff
